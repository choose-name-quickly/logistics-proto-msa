...
public class GatewayConfig {
    ...
    public RouteLocator customRoutes(RouteLocatorBuilder builder, JwtAuthGatewayFilter authFilter) {
        ...
        return builder.routes()
            ...
            // hub-management
            .route("hub-management-service", r -> r.path("/api/couriers/**")
                .filters(f -> f.filter(authFilter))
                .uri("lb://hub-management-service")
            )
            .route("hub-management-service", r -> r.path("/api/hubmanagers/**")
                .filters(f -> f.filter(authFilter))
                .uri("lb://hub-management-service")
            )
            .route("hub-management-service", r -> r.path("/api/companies/**")
                .filters(f -> f.filter(authFilter))
                .uri("lb://hub-management-service")
            )
            // product-company
            ...
            // slack-message
            .route("slack-message-service", r -> r.path("/api/messages/**")
                .filters(f -> f.filter(authFilter))

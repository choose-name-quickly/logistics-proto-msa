package com.namequickly.logistics.product_company.global.swagger;
...
//            .addSecurityItem(accessTokenRequirement);
    ...
    private void addPageableSchemas(Components components) {
        ...
        Schema<?> properties = new MapSchema().properties(
            Map.of("page", new IntegerSchema().example(1), "size", new IntegerSchema().example(10),
...
/*    private void addSecuritySchemes(Components components) {
    ...
    private SecurityScheme accessTokenSecurityScheme() {
        return new SecurityScheme().name(JwtUtil.ACCESS_TOKEN_HEADER) // Authorization 을 키로 함
    ...
    private Info info() {
        return new Info().title("API 명세서").version("0.0.1");
    }
}
